...
class InformedRRTStar:
    ...
    def informed_rrt_star_search(self, animation=True):
        ...
        for i in range(self.max_iter):
            ...
            graph_image = np.zeros((200, 200, 3), dtype=np.uint8)
            
            cv2.circle(graph_image, (10*int(rnd[0]), 10*int(rnd[1])), int(5), (255, 255, 0), -1)

            cv2.imshow("Graph", graph_image)
            cv2.waitKey(1)  # Add a short delay for visualization
            return []
    ...
    def draw_graph(self, x_center=None, c_best=None, c_min=None, e_theta=None,
        rnd=None):
        graph_image = np.zeros((200, 200, 3), dtype=np.uint8) # Create a blank image

        for node in self.node_list:
            if node.parent is not None:
                if node.x or node.y is not None:
                    cv2.line(graph_image, (int(node.x), int(node.y)),
                    (int(self.node_list[node.parent].x), int(self.node_list[node.parent].y)), (0, 255, 0), 2)

        for (ox, oy, size) in self.obstacle_list:
            cv2.circle(graph_image, (10*int(ox), 10*int(oy)), int(10 * size), (255, 0, 0), -1)

        cv2.circle(graph_image, (int(self.start.x), int(self.start.y)), 5, (0, 0, 255), -1)
        cv2.circle(graph_image, (int(self.goal.x), int(self.goal.y)), 5, (0, 0, 255), -1)
        cv2.rectangle(graph_image, (0, 0), (800, 800), (255, 255, 255), -1)  # Set background to white
    ...
    #@staticmethod
    def plot_ellipse(self, x_center, c_best, c_min, e_theta):  # pragma: no cover
